...
from util.argparse_action import ListToNparray

# Default flag value
BATCH_SIZE = 10
DATA_DIRECTORY = '{}/VOCdevkit'.format(expanduser("~"))
...
WEIGHT_DECAY = 0.0005
# VOC's RGB mean value for normalization
...
def get_arguments():
    ...
    parser.add_argument("--rgb-mean", type=float, nargs='+', action=ListToNparray,
                        default=IMG_MEAN,
                        help="A list indicate the mean RGB value in the images.")
...
def main():
    ...
    # Image summary.
    images_summary = tf.py_func(inv_preprocess, [image_batch, args.save_num_images, args.rgb_mean], tf.uint8)
